...
def file_decrypt(filename, subkey_decrypt, filename2store):
    ...
    if chr(plain_text[-1]).isdigit() and (int(chr(plain_text[-1])) - 1) * b'0' == plain_text[-int(chr(plain_text[-1])):-1]:
...
def MAC_code(filename, subkey_encrypt):
    IV = b'0' * 8
...
def MAC():
    ...
    # 获取加密子密钥
    subkey_encrypt = get_subkey(key_bin)
    MAC_code(filename, subkey_encrypt)
